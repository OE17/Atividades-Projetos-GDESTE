...
function App() {
  ...
  const handleReserveSpot = () => {
    ...
    const isDuplicatePlate = reservations.some(reservation => 
      normalizeText(reservation.licensePlate) === normalizedPlate
    );

    if (isDuplicatePlate) {
      alert('Essa placa já está registrada! Cada reserva deve ter uma placa única.');
      return;
    }
  ...
  return (
    <div className="App">
      ...
      {showReservationForm && (
        <div className="modal">
          <h2>Reservar Vaga</h2>
          <input type="text" placeholder="Nome do proprietário" value={ownerName} onChange={(e) => setOwnerName(e.target.value)} />
          <input type="text" placeholder="Modelo do carro" value={carModel} onChange={(e) => setCarModel(e.target.value)} />
          <input type="text" placeholder="Placa do carro" value={licensePlate} onChange={(e) => setLicensePlate(e.target.value)} />
      ...
      {showCancelForm && (
        <div className="modal">
          ...
          <input type="text" placeholder="ID da vaga" value={spotId} onChange={(e) => setSpotId(e.target.value)} />
            ...
            <tbody>
              {reservations.map(reservation => (
                <tr key={reservation.id}>
                  ...
                  <td>
                    <QRCodeSVG value={reservation.id} size={50} />
                  </td>
                </tr>
              ))}
            </tbody>
